...
public interface NewsCorner2Repository extends JpaRepository<NewsCorner2, Long> {
	...
	@Query(value = "select b.*,"
			...
			+ " where (b.category_id = ?1 or ?1 = 0) and b.is_deleted = 0 and b.is_public = 1"
			+ " order by ?4 , publish_at desc limit ?2 , ?3" , nativeQuery = true)
	List<NewsCorner2> searchNewsCorner2ByCategoryId(Long categoryId, Integer limitStart, Integer limitEnd ,String orderBy);
	...
	*/
	...
	Object getDetailsById(Long newscorner2Id, Long userId);
	...
	Optional<NewsCorner2> findByIdAndIsDeleted(Long newscorner2Id,Integer isDeleted);
	...
	List<NewsCorner2> getRelated(Long newscorner2Id);
	
	
	
	@Query(value = "select b.* from newscorner2_post b"
			+ " where b.id = ?1" , nativeQuery = true)//b.is_deleted = 0 and b.is_public = 1 and
	Optional<NewsCorner2> checkNewsCorner2Status(Long newscorner2Id);
